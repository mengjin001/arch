...
from randomgen import Xoshiro512StarStar
...
MAX_MEMORY = 2 ** 29  # 512 miB


def block(rg: RandomGenerator, trend: str, sample_sizes: List[int], cross_section_size: int,
          simulations: int, idx: int):
    ...
    max_blocks_size = MAX_MEMORY // (max_t * max_k * 8)
    ...
    while remaining:
        ...
        for j, sub_size in enumerate(sample_sizes):
            ...
            errors[:, :, :1] = sub[:, :, :1]
            rhs = sub[:, :, 1:]
            ip = (np.transpose(rhs, (0, 2, 1)) @ rhs) / sub_size
            ip_chol_inv = np.linalg.inv(np.linalg.cholesky(ip))
            orth = rhs @ np.transpose(ip_chol_inv, (0, 2, 1))
        ...
        print(f'Estimated time remaining: {str(dt.timedelta(seconds=remaining_time))}')
...
if __name__ == '__main__':
    gen = [RandomGenerator(Xoshiro512StarStar(123456789).jump(i)) for i in range(EX_NUM)]
    sample_sizes = (20, 25, 30, 35, 40, 45, 50, 60, 70, 80, 90, 100, 120, 140, 160,
    ...
    simulations = EX_SIZE
    njobs = cpu_count() - 1
    partial_block = partial(block, sample_sizes=sample_sizes,
                            cross_section_size=cross_section_size,
                            simulations=simulations)

    inputs = product(enumerate(gen), trends)
    Parallel(verbose=50, n_jobs=njobs)(delayed(partial_block)(rg, trend=trend, idx=idx)
                                       for ((idx, rg), trend) in inputs)
